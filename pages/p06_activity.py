...
def render():
    ...
    st.markdown("<div class='page-sub'>Chronological stream of engineering events</div>", unsafe_allow_html=True)
    ...
    if feed.empty:
        st.info("No activity found for current filters.")
        return
    ...
    with ctrl3:
        repo_filter = st.text_input("Filter by repo", key="feed_repo")
    with ctrl4:
        actor_filter = st.text_input("Filter by author", key="feed_actor")
    ...
    if days_back < 9999:
        ...
        df = df[df["ts"] >= cutoff]
    if repo_filter:
        df = df[df["repo"].str.contains(repo_filter, case=False, na=False)]
    if actor_filter:
        df = df[df["actor"].str.contains(actor_filter, case=False, na=False)]

    st.markdown(f"**{len(df):,} events** in feed")
    st.markdown("---")

    PAGE        = 50
    total_pages = max(1, (len(df) - 1) // PAGE + 1)
    pg          = st.number_input("Page", 1, total_pages, 1, key="feed_page")
    page_df     = df.iloc[(pg - 1) * PAGE : pg * PAGE]

    items_html = ""
    for _, row in page_df.iterrows():
        icon  = _ICONS.get(row["type"], "•")
        ts    = row["ts"].strftime("%b %d, %Y  %H:%M UTC") if pd.notna(row["ts"]) else ""
        url   = row.get("url", "")
        title = row.get("title", "")
        link  = (
        ...
        <div class="feed-item">
          ...
          <div class="feed-body">
            <div class="feed-title">{link}&nbsp;<span style="color:#585b70;font-size:.76rem">{row.get('meta','')}</span></div>
            <div class="feed-meta">
              <strong style="color:#a6adc8">{row.get('repo','')}</strong>
              &nbsp;·&nbsp;{row.get('actor','')}
              &nbsp;·&nbsp;{ts}
              &nbsp;·&nbsp;<span style="color:#45475a">{row['type'].replace('_',' ')}</span>
            </div>
          </div>
        </div>"""

    st.markdown(f'<div class="feed-container">{items_html}</div>', unsafe_allow_html=True)
    st.caption(f"Page {pg} of {total_pages} · {len(df):,} events total")
