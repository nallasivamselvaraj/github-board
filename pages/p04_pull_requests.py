...
_COL_CFG = {
    "pr_url":          st.column_config.LinkColumn("Link", display_text="Open ↗"),
    "age_days":        st.column_config.NumberColumn("Age (days)", format="%d d"),
    "cycle_time_days": st.column_config.NumberColumn("Cycle (days)", format="%.1f d"),
    "additions":       st.column_config.NumberColumn("➕ Added"),
    "deletions":       st.column_config.NumberColumn("➖ Removed"),
...
def _table(df: pd.DataFrame, key_prefix: str, filename: str):
    if df.empty:
        st.markdown(
            f"<div class='accent-card' style='text-align:center;color:#9ca3af;padding:24px'>"
            f"No {filename.replace('_', ' ')} in current selection.</div>",
    ...
    with c3:
        q = st.text_input("🔍 Quick filter", key=f"prq_{key_prefix}", placeholder="Title or author…")
    if q:
        mask = (
    ...
    with right:
        pg = st.number_input("Page", 1, total, 1, key=f"prp_{key_prefix}", label_visibility="collapsed")
    with left:
...
def render():
    st.markdown("<div class='page-header'>🔀 Pull Requests</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>Open, closed, draft, merged, and review-pending PRs</div>",
                unsafe_allow_html=True)
    ...
    if prs.empty:
        st.markdown(
            """<div class='info-card' style='border-color:rgba(196,22,42,0.3)'>
                ...
                <div style='font-size:0.85rem;color:#718096'>No PR data found — click <b>🔄 Refresh Data</b> in the sidebar.</div>
            </div>""",
            unsafe_allow_html=True,
        )
        return
    ...
    merge_rate = round(len(merged_prs) / len(prs) * 100, 1) if len(prs) else 0

    # ── KPI row ────────────────────────────────────────────
    st.markdown("<div class='kpi-group'>🔀 Pull Request Summary</div>", unsafe_allow_html=True)
    ...
    # ── Charts ─────────────────────────────────────────────
    st.markdown("<div class='section-header'>📊 PR Analytics</div>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        st.plotly_chart(charts.pr_state_pie(prs), use_container_width=True)
    with col2:
        st.plotly_chart(charts.pr_repo_bar(prs), use_container_width=True)
    ...
    # ── Tabbed tables ──────────────────────────────────────
    st.markdown("<div class='section-header'>📋 PR Details</div>", unsafe_allow_html=True)
    tabs = st.tabs([
        f"🟢 Open ({len(open_prs):,})",
        f"✅ Closed ({len(closed_prs):,})",
        f"📝 Draft ({len(draft_prs):,})",
        f"🟣 Merged ({len(merged_prs):,})",
        f"👁 Review Ready ({len(review_prs):,})",
    ])
    ...
    with tabs[4]:
        st.markdown(
            "<div class='accent-card' style='margin-bottom:10px'>"
            "Open PRs <b>not</b> in draft — ready for review or currently under review.</div>",
            unsafe_allow_html=True,
        )
