...
def render():
    ...
    st.markdown("<div class='page-sub'>Organisation-wide engineering health at a glance</div>", unsafe_allow_html=True)
    ...
    with col4:
        if not prs.empty:
            ...
            st.plotly_chart(charts.release_timeline(releases), width="stretch")

    st.plotly_chart(charts.engineering_activity_timeline(issues, prs), width="stretch")
    ...
    if not rs.empty:
        st.dataframe(rs.head(15), width="stretch", height=340)
